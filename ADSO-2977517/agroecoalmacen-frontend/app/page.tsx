...
export default function DashboardInventario() {
  ...
  useEffect(() => {
    async function fetchOrganismos() {
      ...
      if (mapped.length > 0) seleccionarOrganismo(mapped[0]);
    }
    fetchOrganismos();
  }, []);
  ...
  return (
    <div style={{ fontFamily: "Arial, sans-serif", overflow: "hidden" }}>
      ...
      >
        <div className="sidebar-titulo">
          ...
          <style jsx>{`
            ...
            @keyframes glow {
              0% { text-shadow: 0 0 5px #8360c3,0 0 10px #6a0dad; color:#007e69ff; }
            ...
            {mostrarFormulario && (
              <div style={{
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 5 }}>
                  ...
                  <button onClick={cancelarFormulario} style={{ background: "#bdc3c7", color: "#000", padding: 6, borderRadius: 6 }}>❌ Cancelar</button>
                </div>
              </div>
            )}
      ...
      <aside style={{
        ...
        <style>{`
          .alert { padding: 20px; margin: 8px 0; border-radius:50px; font-size: 13px; font-weight: bold; color: #fff; }
          .alert.roja { background: #e74c3c; }
          .alert.amarilla { background: #f1c40f; color:#000; }
          .alert.verde { background: #2ecc71; }
          .alerta-animada { font-size: 24px; font-weight: bold; color: #fe9b9bff; text-align: center; animation: brilloAlert 0.3s ease-in-out infinite alternate; text-shadow: 0 0 5px #e7785cff, 0 0 10px #ec5700ff; }
      ...
      {/* BOTÓN PANEL DERECHO */}
      <button onClick={() => setRightMenuOpen(!rightMenuOpen)} style={{
        position: "fixed",
        top: 54,
